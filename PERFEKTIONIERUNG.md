...
- **✅ S0.1 — Test-Netz Suite-Kern.** *(erledigt 4. Juli 2026)* Node-Headless-Selbsttest (`test/selftest-node.js` im Root, Muster wie `pdf/test`) + `selfTest()` in `app.js`. Deckt **32 Prüfungen**: Geld (chf/money/chfShort), Datum (fmtDate/addDays/isoOf/dayDiff/parseDateFlexible inkl. Monats-/Saison-/Ungültig-Fälle), BKP (parseBkp/bkpBase/bkpCmp), Domäne (teilSumme, gewerkeSorted), UID/esc. **DoD erreicht:** `node test/selftest-node.js` → 32/32 grün. *Nächster Ausbau: Kostensummen, Nachträge, Zahlungsplan-Rechnung.*
- **S0.2 — Test-Netz Submit Paper.** Gleiches für `write.js`: Calc-Formeln (`evalCell`), htmlToGrid/gridToHtml, PDF→Paper-Konvertierung (parseNum/checkCalc/Tabellen). **DoD:** eigener Headless-Test grün.
- **S0.3 — Design-System zentralisieren.** Eine gemeinsame Token-Datei (`ui/tokens.css`) mit Farben/Schatten/Radien/Abständen; Suite + PDF + Paper ziehen daraus. Rest der PDF↔Paper-Vereinheitlichung abschliessen (Toolbar/Ribbon, Zustände, Popups). **DoD:** identische Tokens in allen drei Apps, ein sichtbar einheitlicher Look.
