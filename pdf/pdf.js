...
function drawOpening(svg, a, arr) {
  ...
  if (seld) for (const st of revHits) { const gc = revealGroupColor(a, st.edge, st.side === 'i' ? 'in' : 'out'); svg.appendChild(attachTip(addHoverHL(svgEl('polygon', { class: 'rev-hit', points: st.poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: gc, 'fill-opacity': 0.08, stroke: gc, 'stroke-width': 1, 'stroke-opacity': 0.75, 'stroke-dasharray': '4 3', 'vector-effect': 'non-scaling-stroke', 'data-id': a.id, 'data-rev': st.edge + ':' + st.side + ':' + st.li })), revealName(st.edge, st.side, st.li))); }   // Laibungs-Schichten: gestrichelte Gruppenfarben-Kontur + ganz leichte Füllung (Mittelweg) + Hover-Name
