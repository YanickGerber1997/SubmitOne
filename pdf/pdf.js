...
function drawOpening(svg, a, arr) {
  ...
  if (seld) { const seen = {}; const edgeStrip = (A, B, attr, tip, color) => { const dx = B[0] - A[0], dy = B[1] - A[1], L = Math.hypot(dx, dy) || 1, ox = -dy / L * 5, oy = dx / L * 5, poly = [[A[0] + ox, A[1] + oy], [B[0] + ox, B[1] + oy], [B[0] - ox, B[1] - oy], [A[0] - ox, A[1] - oy]]; const z = svgEl('polygon', Object.assign({ class: 'edge-strip', points: poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: color || '#1a9a4e', 'fill-opacity': 0.28, stroke: color || '#1a9a4e', 'stroke-width': 1.6, 'stroke-opacity': 0.95, 'vector-effect': 'non-scaling-stroke', 'data-id': a.id }, attr)); svg.appendChild(attachTip(addHoverHL(z), tip)); };   // SICHTBARE, farbcodierte Ziehkante (breitere Hit-Zone, Hover-Glow), Tooltip sagt was sie tut
    for (const st of revHits) { const q = st.poly; if (q.length < 4) continue; const sgn = st.edge === 'R' ? 1 : -1, ca = Math.cos(a.ang || 0), sa = Math.sin(a.ang || 0), key = st.edge + ':' + st.side + ':' + st.li, nm = revealName(st.edge, st.side, st.li);
      const k2 = st.edge + ':' + st.side; if (!seen[k2]) { seen[k2] = 1; _revtHandles[a.id + '|' + key] = { dirx: -sgn * ca, diry: -sgn * sa }; edgeStrip(q[0], q[3], { 'data-revt': key }, nm + ' · Rahmen sichtbar ziehen', '#1a9a4e'); }   // grün = Lappung (Rahmen sichtbar)
      const fmx = (q[0][0] + q[3][0]) / 2, fmy = (q[0][1] + q[3][1]) / 2, flx = (q[2][0] + q[3][0]) / 2, fly = (q[2][1] + q[3][1]) / 2, odx = flx - fmx, ody = fly - fmy, odl = Math.hypot(odx, ody) || 1; _revoHandles[a.id + '|' + key] = { dirx: odx / odl, diry: ody / odl, li: st.li }; edgeStrip(q[2], q[3], { 'data-revo': key }, nm + ' · Überstand ziehen', '#d4881f');   // orange = Überstand
      const tdx = q[1][0] - q[0][0], tdy = q[1][1] - q[0][1], tdl = Math.hypot(tdx, tdy) || 1; _revThickHandles[a.id + '|' + key] = { dirx: tdx / tdl, diry: tdy / tdl }; edgeStrip(q[1], q[2], { 'data-revtk': key }, nm + ' · Dicke ziehen', '#2980b9');   // blau = Dicke
      const ldx = q[0][0] - q[3][0], ldy = q[0][1] - q[3][1], ldl = Math.hypot(ldx, ldy) || 1; _revLenHandles[a.id + '|' + key] = { dirx: ldx / ldl, diry: ldy / ldl, depthCm: Math.round(ptsToCm(ldl) * 10) / 10 }; edgeStrip(q[0], q[1], { 'data-revln': key }, nm + ' · Tiefe/Länge ziehen', '#8e44ad');   // violett = Tiefe/Länge
    }
    ...
    for (const f of (P.fills || [])) { if (f.role !== 'frame') continue; const q = f.poly; if (q.length < 4) continue;
      ...
      edgeStrip(inner[0], inner[1], { 'data-dim': key }, 'Rahmen · Breite ziehen', '#37404a');   // anthrazit = Rahmenbreite
    } }
