...
function drawOpening(svg, a, arr) {
  ...
  if (seld) { const seen = {}; for (const st of revHits) { const k2 = st.edge + ':' + st.side; if (seen[k2]) continue; seen[k2] = 1; const q = st.poly; if (q.length < 4) continue; const fmx = (q[0][0] + q[3][0]) / 2, fmy = (q[0][1] + q[3][1]) / 2, sgn = st.edge === 'R' ? 1 : -1, ca = Math.cos(a.ang || 0), sa = Math.sin(a.ang || 0), key = st.edge + ':' + st.side + ':' + st.li; _revtHandles[a.id + '|' + key] = { dirx: -sgn * ca, diry: -sgn * sa }; const c = svgEl('circle', { cx: fmx.toFixed(2), cy: fmy.toFixed(2), r: 5, fill: '#cfe6c4', stroke: '#1a9a4e', 'stroke-width': 2, 'data-id': a.id, 'data-revt': key, 'vector-effect': 'non-scaling-stroke' }); c.style.cursor = 'ew-resize'; svg.appendChild(c);   // Lappung (Rahmen sichtbar)
    const flx = (q[2][0] + q[3][0]) / 2, fly = (q[2][1] + q[3][1]) / 2, odx = flx - fmx, ody = fly - fmy, odl = Math.hypot(odx, ody) || 1; _revoHandles[a.id + '|' + key] = { dirx: odx / odl, diry: ody / odl, li: st.li }; const c2 = svgEl('circle', { cx: flx.toFixed(2), cy: fly.toFixed(2), r: 4.5, fill: '#f6d6a8', stroke: '#d4881f', 'stroke-width': 2, 'data-id': a.id, 'data-revo': key, 'vector-effect': 'non-scaling-stroke' }); c2.style.cursor = 'crosshair'; svg.appendChild(c2);   // Überstand (über die Wandfläche) – oranger Griff an der Wandflächenkante
  } }
