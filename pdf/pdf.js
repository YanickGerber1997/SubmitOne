...
function openingCutPoly(o) {   // Laibungs-bewusster Ausschnitt (H-Form): die Wand lappt an den Laibungstiefen auf den Rahmen – pro Jamb (L/R) und Tiefe (innen/aussen) eigene freie Breite
  ...
  const free = (edge, side) => { const bv = (o.reveals && o.reveals[edge] && o.reveals[edge].boardVis != null) ? o.reveals[edge].boardVis : bvG, lapE = Math.max(0, Math.min(hw * 0.92, frameW - cmToPts(bv))); return Math.max(2, hw - Math.max(0, lapE - openingRevealTotalPts(o, edge, side))); };   // freie Halbbreite je Kante: Öffnung − Wand-Lappung (Lappung = Rahmen − „Rahmen sichtbar" dieser Kante)
...
function openingRevealStrips(a, arr) {   // Laibung: 1,5 cm Rahmen sichtbar → Schalung 1,5 cm (Schraffur) → Rest Dämmung bis Rahmen; innen Putz/Brett
  ...
  const drawReveal = (layers, sideOut, sgn, slopeCm, bv, edge) => {   // EINE Laibungsseite: dünne Laibung am Rahmen; bv = „Rahmen sichtbar" dieser Kante; tag jede Schicht (edge/side/li) für Klick im Grundriss
  ...
  for (const sgn of [-1, 1]) {   // links (sgn -1, Kante L) und rechts (sgn +1, Kante R) je eigene Laibung; Fallback = globale Liste
    const edge = sgn < 0 ? 'L' : 'R', er = a.reveals && a.reveals[edge], slope = (er && er.slope) || 0, bv = er && er.boardVis != null ? er.boardVis : boardVisCm;
    if (anType !== 'innen') drawReveal(openingEdgeLayers(a, edge, 'i') || innerLayers, false, sgn, slope, bv, edge);
    if (anType !== 'aussen') drawReveal(openingEdgeLayers(a, edge, 'o') || outerLayers, true, sgn, slope, bv, edge);
  }
  if (anType !== 'none') {
...
function openRevealLayerPop(pv, a, revAttr, cx, cy) {   // Inline-Editor für EINE Laibungsschicht (im Grundriss angeklickt): Material/Dicke + „Rahmen sichtbar" dieser Kante
  ...
  const eachEdge = cb => { const es = revealLinked ? ['L', 'R', 'T', 'B'] : [edge]; for (const e of es) { a.reveals[e] = a.reveals[e] || {}; if (!Array.isArray(a.reveals[e][sk])) a.reveals[e][sk] = []; cb(a.reveals[e], a.reveals[e][sk]); } };   // gekoppelt = alle Kanten, sonst nur diese
  ...
  const lk = document.createElement('button'); lk.style.cssText = 'width:100%;margin-bottom:7px;padding:4px;cursor:pointer;border:1px solid #b8c0ad;border-radius:6px;background:' + (revealLinked ? '#e6f0e2' : '#fff'); const lkTxt = () => lk.textContent = revealLinked ? '🔗 alle Laibungen gekoppelt' : '🔓 nur diese Kante'; lkTxt(); lk.title = 'Standard: eine ändern = alle 4 Laibungen. Klick = nur diese Kante bearbeiten'; lk.onclick = () => { revealLinked = !revealLinked; lkTxt(); lk.style.background = revealLinked ? '#e6f0e2' : '#fff'; }; pop.appendChild(lk);
  ...
  const ms = document.createElement('select'); matOpts.forEach(([k, lab]) => { const o = document.createElement('option'); o.value = k; o.textContent = lab; if (k === L.mat) o.selected = true; ms.appendChild(o); }); ms.onchange = () => { eachEdge((er, l2) => { if (l2[li]) l2[li].mat = ms.value; }); upd(); }; mk('Material', ms);
  const tn = document.createElement('input'); tn.type = 'number'; tn.min = '0.1'; tn.max = '30'; tn.step = '0.1'; tn.value = L.t; tn.style.width = '62px'; tn.onchange = () => { const v = parseFloat((tn.value || '').replace(',', '.')); if (v > 0) { eachEdge((er, l2) => { if (l2[li]) l2[li].t = v; }); upd(); } }; mk('Dicke (cm)', tn);
  const ln = document.createElement('input'); ln.type = 'number'; ln.min = '0'; ln.max = '60'; ln.step = '0.5'; ln.value = (L.len != null ? L.len : ''); ln.placeholder = 'bis Rahmen'; ln.style.width = '62px'; ln.title = 'Länge/Tiefe der Schicht (cm, von der Wandfläche). Leer = bis zum Rahmen'; ln.onchange = () => { const v = parseFloat((ln.value || '').replace(',', '.')), nv = (ln.value === '' || isNaN(v)) ? undefined : Math.max(0, v); eachEdge((er, l2) => { if (l2[li]) l2[li].len = nv; }); upd(); }; mk('Länge/Tiefe (cm)', ln);
  const ov = document.createElement('input'); ov.type = 'number'; ov.min = '-20'; ov.max = '20'; ov.step = '0.1'; ov.value = (L.over != null ? L.over : 0); ov.style.width = '62px'; ov.title = 'Überstand (+) über die Wandfläche / Rücksprung (−) dahinter (cm)'; ov.onchange = () => { const v = parseFloat((ov.value || '').replace(',', '.')), nv = isNaN(v) ? 0 : v; eachEdge((er, l2) => { if (l2[li]) l2[li].over = nv; }); upd(); }; mk('Überstand +/− (cm)', ov);
  const bvv = document.createElement('input'); bvv.type = 'number'; bvv.min = '0'; bvv.max = '30'; bvv.step = '0.1'; bvv.value = (a.reveals[edge].boardVis != null ? a.reveals[edge].boardVis : (a.boardVis != null ? a.boardVis : 1)); bvv.style.width = '62px'; bvv.onchange = () => { const v = parseFloat((bvv.value || '').replace(',', '.')), nv = isNaN(v) ? 1 : Math.max(0, v); eachEdge(er => { er.boardVis = nv; }); upd(); }; mk('Rahmen sichtbar (cm)', bvv);
