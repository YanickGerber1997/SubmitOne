...
function openingRevealStrips(a, arr) {   // Laibung: 1,5 cm Rahmen sichtbar → Schalung 1,5 cm (Schraffur) → Rest Dämmung bis Rahmen; innen Putz/Brett
  ...
  const userLin = Array.isArray(a.revealLining) && a.revealLining.length ? a.revealLining.map(L => [L.mat, L.t, (L.depth == null ? 1 : L.depth), (L.protrude || 0)]) : null;   // eigene Laibungsschichten innen (Material/Dicke/Tiefe/Überstand)
  const userLinOut = Array.isArray(a.revealLiningOut) && a.revealLiningOut.length ? a.revealLiningOut.map(L => [L.mat, L.t, (L.depth == null ? 1 : L.depth), (L.protrude || 0)]) : null;   // eigene Laibungsschichten aussen
