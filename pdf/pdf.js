...
function fillWallList(bodyEl) {   // Wandaufbau/U-Werte in das Listen-Panel
  ...
  groups.forEach((g, i) => { const u = wallUValue(g.layers, g.uVal), us = (Math.round(u * 1000) / 1000).toFixed(3), comp = g.uVal != null, uc = u < 0.2 ? 'uval-good' : u < 0.35 ? 'uval-mid' : 'uval-poor'; rows += '<tr><td><b>W' + (i + 1) + '</b></td><td>' + buildupThumb(g.layers, g.totalCm) + '</td><td>' + desc(g.layers) + '</td><td class="num" style="text-align:right">' + (Math.round(g.totalCm * 10) / 10) + '</td><td><input class="bu-u ' + uc + '" data-sig="' + g.sig.replace(/"/g, '') + '" type="number" step="0.01" min="0.05" value="' + us + '">' + (comp ? ' ✎' : '') + '</td><td style="text-align:center">' + g.walls.length + '</td></tr>'; csv += desc(g.layers) + '\t' + (Math.round(g.totalCm * 10) / 10) + '\t' + us + '\t' + g.walls.length + '\n'; });
