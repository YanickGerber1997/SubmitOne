...
function drawOpening(svg, a, arr) {
  ...
  if (seld) for (const st of revHits) { const gc = revealGroupColor(a, st.edge, st.side === 'i' ? 'in' : 'out'); svg.appendChild(addHoverHL(svgEl('polygon', { class: 'rev-hit', points: st.poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: 'transparent', stroke: gc, 'stroke-width': 1, 'stroke-opacity': 0.7, 'stroke-dasharray': '4 3', 'data-id': a.id, 'data-rev': st.edge + ':' + st.side + ':' + st.li }))); }   // Laibungs-Schichten: nur dünne, gestrichelte Gruppenfarben-Kontur (minimal), volle Hervorhebung erst beim Hover
  ...
  if (seld) { const seen = {}; const edgeStrip = (A, B, color, attr) => { const dx = B[0] - A[0], dy = B[1] - A[1], L = Math.hypot(dx, dy) || 1, ox = -dy / L * 3, oy = dx / L * 3, poly = [[A[0] + ox, A[1] + oy], [B[0] + ox, B[1] + oy], [B[0] - ox, B[1] - oy], [A[0] - ox, A[1] - oy]]; const z = svgEl('polygon', Object.assign({ points: poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: color, 'fill-opacity': 0.9, stroke: 'none', 'data-id': a.id }, attr)); z.style.cursor = 'grab'; svg.appendChild(z); };
...
function drawSection(svg, a, arr) {
  const revLayer = () => { const rh = _secCache[a.id] && _secCache[a.id]._revHits; if (rh) for (const h of rh) { const at = { class: h.frame ? 'frame-hit' : 'rev-hit', points: h.pts.map(q => q[0].toFixed(2) + ',' + q[1].toFixed(2)).join(' '), fill: h.gcol || 'transparent', 'data-id': h.oid }; if (h.gcol) { at.fill = 'transparent'; at.stroke = h.gcol; at['stroke-width'] = 1; at['stroke-opacity'] = 0.7; at['stroke-dasharray'] = '4 3'; } if (h.frame) at['data-frame'] = '1'; else at['data-rev'] = h.rev; svg.appendChild(addHoverHL(svgEl('polygon', at))); } };   // klickbare Laibungsschichten (Sturz/Schwelle) + Rahmen, farbcodiert nach Zugehörigkeit (konsistent zum Grundriss)
