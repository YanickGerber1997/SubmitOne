...
function drawOpening(svg, a, arr) {
  ...
  if (seld) { const seen = {}, lastByGroup = {}; const edgeStrip = (A, B, attr, tip, color) => { const dx = B[0] - A[0], dy = B[1] - A[1], L = Math.hypot(dx, dy) || 1, ox = -dy / L * 5, oy = dx / L * 5, poly = [[A[0] + ox, A[1] + oy], [B[0] + ox, B[1] + oy], [B[0] - ox, B[1] - oy], [A[0] - ox, A[1] - oy]]; const z = svgEl('polygon', Object.assign({ class: 'edge-strip', points: poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: color, 'fill-opacity': 0, stroke: color, 'stroke-width': 2.4, 'stroke-opacity': 0, 'vector-effect': 'non-scaling-stroke', 'data-id': a.id }, attr)); svg.appendChild(attachTip(addHoverHL(z), tip)); };   // unsichtbare Ziehkante; beim Drüberfahren leuchtet die Linie in ihrer Gruppenfarbe leicht auf (CSS .edge-strip.hl-on)
    ...
    const pmBtn = (px0, py0, sym, attr, tip, col) => { const c = svgEl('circle', Object.assign({ cx: px0.toFixed(2), cy: py0.toFixed(2), r: 6, fill: '#fff', stroke: col, 'stroke-width': 1.6, 'vector-effect': 'non-scaling-stroke', 'data-id': a.id }, attr)); c.style.cursor = 'pointer'; const t = svgEl('text', { x: px0.toFixed(2), y: (py0 + 0.3).toFixed(2), 'text-anchor': 'middle', 'dominant-baseline': 'central', 'font-size': 9, 'font-weight': 700, fill: col, 'pointer-events': 'none' }); t.textContent = sym; svg.appendChild(attachTip(c, tip)); svg.appendChild(t); };
    for (const k2 in lastByGroup) { const st = lastByGroup[k2], q = st.poly, gc = revealGroupColor(a, st.edge, st.side === 'i' ? 'in' : 'out'), nm = revealName(st.edge, st.side);   // + / − an der innersten Schicht (inneres Ende = sOut-Kante q[1]-q[2])
      const mx = (q[1][0] + q[2][0]) / 2, my = (q[1][1] + q[2][1]) / 2, ex = q[2][0] - q[1][0], ey = q[2][1] - q[1][1], el = Math.hypot(ex, ey) || 1, ux2 = ex / el, uy2 = ey / el;
      const tx = a.x - mx, ty = a.y - my, td = Math.hypot(tx, ty) || 1, tnx = tx / td, tny = ty / td, bx = mx + tnx * 10, by = my + tny * 10, key = st.edge + ':' + st.side + ':' + st.li;
      pmBtn(bx - ux2 * 8, by - uy2 * 8, '+', { 'data-revadd': key }, nm + ' · Schicht einfügen', gc);
      pmBtn(bx + ux2 * 8, by + uy2 * 8, '−', { 'data-revdel': key }, nm + ' · Schicht entfernen', gc);
    }
