...
function sectionCutOpening(out, X, Yh, distPt, appW, o, H, perPt, wall, flip, noDims, mullion, revealOnly) {   // revealOnly = nur Laibung/Schichteinzug zeichnen (Rahmen/Glas/Bank kommen aus Solids)
  ...
  if (layered) {   // Laibung = voller Wandaufbau (wie im Grundriss): Mauerwerk+Dämmung+Putz laufen als Sturz-Untersicht / Brüstungs-Oberseite (schraffiert) bis zum Rahmen; Lining-Lappung + Rahmen/Glas zeichnen darüber
    ...
    for (const [yA, yB] of bands) { const yT = Math.min(yA, yB), hh = Math.abs(yB - yA); if (hh < 1) continue; let acc = 0; for (const L of wls) { const m0 = -1 + 2 * acc / tot, m1 = -1 + 2 * (acc + L.t) / tot; acc += L.t; const xa = cx + ht2 * m0, xb = cx + ht2 * m1, x = Math.min(xa, xb), wd = Math.abs(xb - xa), mt = WALL_MATS[L.mat] || {}; if (wd < 0.3 || L.mat === 'luft') continue; out.push({ t: 'rect', x, y: yT, w: wd, h: hh, fill: mt.fill || '#fff', stroke: mt.color || '#1c242c', sw: 0.4 }); if (!simpleMode && mt.hatch) sectionBandHatch(out, x, yT, wd, hh, L.mat, null); } }
  }
