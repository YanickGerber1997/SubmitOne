...
function openingRevealStrips(a, arr) {   // Laibung: 1,5 cm Rahmen sichtbar → Schalung 1,5 cm (Schraffur) → Rest Dämmung bis Rahmen; innen Putz/Brett
  ...
  const userLin = Array.isArray(a.revealLining) && a.revealLining.length ? a.revealLining.map(L => [L.mat, L.t, (L.depth == null ? 1 : L.depth), (L.protrude || 0), L.dF, L.dS]) : null;   // innen: Material/Dicke/Tiefe/Überstand/AbstandRahmen(dF)/AbstandWand(dS)
  const userLinOut = Array.isArray(a.revealLiningOut) && a.revealLiningOut.length ? a.revealLiningOut.map(L => [L.mat, L.t, (L.depth == null ? 1 : L.depth), (L.protrude || 0), L.dF, L.dS]) : null;   // aussen
  ...
  if (anType !== 'innen' && Math.abs((fmA - oneCm) + 1) > 0.02) { let sAcc = 0; const mEndFull = fmA - oneCm; for (const [mat, tcm, dep, prot, dF, dS] of innerLining) { const mt = LINING_MAT[mat] || WALL_MATS[mat] || {}, tw = Math.min(0.42, cmToPts(tcm) / hw), sk = mt.stroke || mt.color || '#1c242c', mEnd = (dF != null) ? Math.max(-0.98, mEndFull - cmToPts(dF) / ht) : (-1 + Math.max(0.02, Math.min(1, dep)) * (mEndFull + 1)), mStart = -1 - (prot ? cmToPts(prot) / ht : 0) + (dS != null ? cmToPts(dS) / ht : 0); for (const sgn of [-1, 1]) { if (a.noSillReveal && sgn === -1) continue; const s0 = sgn * (1 - sAcc), s1 = sgn * (1 - sAcc - tw); strips.push({ poly: [corner(s0, mStart), corner(s1, mStart), corner(s1, mEnd), corner(s0, mEnd)], fill: mt.fill || '#fff', stroke: sk, hatch: mt.hatch ? bandHatch(Math.min(s0, s1), Math.max(s0, s1), mStart, mEnd, corner, hw, ht, stepS) : null }); } sAcc += tw; } }   // Innen-Laibung: dF=Abstand Rahmen, dS=Abstand Wandfläche, prot=Überstand
  if (userLinOut && anType !== 'aussen' && Math.abs((fmB + oneCm) - 1) > 0.02) { let sAcc = 0; const mEndFull = fmB + oneCm; for (const [mat, tcm, dep, prot, dF, dS] of userLinOut) { const mt = LINING_MAT[mat] || WALL_MATS[mat] || {}, tw = Math.min(0.42, cmToPts(tcm) / hw), sk = mt.stroke || mt.color || '#1c242c', mEnd = (dF != null) ? Math.min(0.98, mEndFull + cmToPts(dF) / ht) : (1 - Math.max(0.02, Math.min(1, dep)) * (1 - mEndFull)), mStart = 1 + (prot ? cmToPts(prot) / ht : 0) - (dS != null ? cmToPts(dS) / ht : 0); for (const sgn of [-1, 1]) { if (a.noSillReveal && sgn === -1) continue; const s0 = sgn * (1 - sAcc), s1 = sgn * (1 - sAcc - tw); strips.push({ poly: [corner(s0, mStart), corner(s1, mStart), corner(s1, mEnd), corner(s0, mEnd)], fill: mt.fill || '#fff', stroke: sk, hatch: mt.hatch ? bandHatch(Math.min(s0, s1), Math.max(s0, s1), mEnd, mStart, corner, hw, ht, stepS) : null }); } sAcc += tw; } }   // Aussen-Laibung: dF=Abstand Rahmen, dS=Abstand Wandfläche
