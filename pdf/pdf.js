...
function drawOpening(svg, a, arr) {
  ...
  if (seld) { const seen = {}, lastByGroup = {}; const edgeStrip = (A, B, attr, tip, color) => { const dx = B[0] - A[0], dy = B[1] - A[1], L = Math.hypot(dx, dy) || 1, ox = -dy / L * 6, oy = dx / L * 6, poly = [[A[0] + ox, A[1] + oy], [B[0] + ox, B[1] + oy], [B[0] - ox, B[1] - oy], [A[0] - ox, A[1] - oy]]; const g2 = svgEl('g', { class: 'edge-strip' }); const z = svgEl('polygon', Object.assign({ points: poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: 'transparent', stroke: 'none', 'data-id': a.id }, attr)); const ln = svgEl('line', { class: 'edge-line', x1: A[0].toFixed(2), y1: A[1].toFixed(2), x2: B[0].toFixed(2), y2: B[1].toFixed(2), stroke: color, 'stroke-width': 2.6, 'stroke-opacity': 0, 'stroke-dasharray': '4 3', 'vector-effect': 'non-scaling-stroke', 'pointer-events': 'none' }); g2.appendChild(z); g2.appendChild(ln); svg.appendChild(attachTip(addHoverHL(g2), tip)); };   // unsichtbare Ziehkante; beim Drüberfahren leuchtet NUR die gestrichelte Linie dieser Kante in ihrer Gruppenfarbe auf
    for (const st of revHits) { const q = st.poly; if (q.length < 4) continue; const sgn = st.edge === 'R' ? 1 : -1, ca = Math.cos(a.ang || 0), sa = Math.sin(a.ang || 0), key = st.edge + ':' + st.side + ':' + st.li, nm = revealName(st.edge, st.side, st.li), gc = revealGroupColor(a, st.edge, st.side === 'i' ? 'in' : 'out');
      const k2 = st.edge + ':' + st.side; if (!seen[k2]) { seen[k2] = 1; _revtHandles[a.id + '|' + key] = { dirx: -sgn * ca, diry: -sgn * sa }; edgeStrip(q[0], q[3], { 'data-revt': key }, nm + ' · Rahmen sichtbar ziehen', gc); }   // Lappung (Rahmen sichtbar)
      const fmx = (q[0][0] + q[3][0]) / 2, fmy = (q[0][1] + q[3][1]) / 2, flx = (q[2][0] + q[3][0]) / 2, fly = (q[2][1] + q[3][1]) / 2, odx = flx - fmx, ody = fly - fmy, odl = Math.hypot(odx, ody) || 1; _revoHandles[a.id + '|' + key] = { dirx: odx / odl, diry: ody / odl, li: st.li }; edgeStrip(q[2], q[3], { 'data-revo': key }, nm + ' · Überstand ziehen', gc);   // Überstand
      const tdx = q[1][0] - q[0][0], tdy = q[1][1] - q[0][1], tdl = Math.hypot(tdx, tdy) || 1; _revThickHandles[a.id + '|' + key] = { dirx: tdx / tdl, diry: tdy / tdl }; edgeStrip(q[1], q[2], { 'data-revtk': key }, nm + ' · Dicke ziehen', gc);   // Dicke
      const ldx = q[0][0] - q[3][0], ldy = q[0][1] - q[3][1], ldl = Math.hypot(ldx, ldy) || 1; _revLenHandles[a.id + '|' + key] = { dirx: ldx / ldl, diry: ldy / ldl, depthCm: Math.round(ptsToCm(ldl) * 10) / 10 }; edgeStrip(q[0], q[1], { 'data-revln': key }, nm + ' · Tiefe/Länge ziehen', gc);   // Tiefe/Länge
      if (lastByGroup[k2] == null || st.li > lastByGroup[k2].li) lastByGroup[k2] = st;   // innerste Schicht je Gruppe merken (für + / −)
    }
    const pmBtn = (px0, py0, sym, attr, tip, col) => { const c = svgEl('circle', Object.assign({ class: 'pm-btn', cx: px0.toFixed(2), cy: py0.toFixed(2), r: 4, fill: '#fff', stroke: col, 'stroke-width': 1.1, 'vector-effect': 'non-scaling-stroke', 'data-id': a.id }, attr)); c.style.cursor = 'pointer'; const t = svgEl('text', { x: px0.toFixed(2), y: (py0 + 0.2).toFixed(2), 'text-anchor': 'middle', 'dominant-baseline': 'central', 'font-size': 6.5, 'font-weight': 700, fill: col, 'pointer-events': 'none' }); t.textContent = sym; svg.appendChild(attachTip(c, tip)); svg.appendChild(t); };
    for (const k2 in lastByGroup) { const st = lastByGroup[k2], q = st.poly, gc = revealGroupColor(a, st.edge, st.side === 'i' ? 'in' : 'out'), nm = revealName(st.edge, st.side);   // + / − klein direkt an der inneren Kante (sOut-Kante q[1]-q[2]) der innersten Schicht
      const mx = (q[1][0] + q[2][0]) / 2, my = (q[1][1] + q[2][1]) / 2, ex = q[2][0] - q[1][0], ey = q[2][1] - q[1][1], el = Math.hypot(ex, ey) || 1, ux2 = ex / el, uy2 = ey / el, key = st.edge + ':' + st.side + ':' + st.li;
      pmBtn(mx - ux2 * 5, my - uy2 * 5, '+', { 'data-revadd': key }, nm + ' · Schicht einfügen', gc);
      pmBtn(mx + ux2 * 5, my + uy2 * 5, '−', { 'data-revdel': key }, nm + ' · Schicht entfernen', gc);
    }
    ...
    for (const f of (P.fills || [])) { if (f.role !== 'frame') continue; const q = f.poly; if (q.length < 4) continue;
      ...
      edgeStrip(inner[0], inner[1], { 'data-dim': key }, 'Rahmen · Breite ziehen', '#37404a');   // Rahmenbreite (anthrazit, nur Hover)
    } }
