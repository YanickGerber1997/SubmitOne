...
function openingCutPoly(o) {   // Laibungs-bewusster Ausschnitt (H-Form): die Wand lappt an den Laibungstiefen auf den Rahmen – pro Jamb (L/R) und Tiefe (innen/aussen) eigene freie Breite
  ...
  const free = (edge, side) => { const er = o.reveals && o.reveals[edge], bv = (er && er.boardVis != null) ? er.boardVis : bvG, grow = er && er.grow ? cmToPts(er.grow) : 0, lapE = Math.max(0, Math.min(hw * 0.92, frameW - cmToPts(bv))); return Math.max(2, hw - Math.max(0, lapE - openingRevealTotalPts(o, edge, side)) + grow); };   // freie Halbbreite je Kante: Öffnung − Wand-Lappung + grow (Wand zurückgeschnitten → Öffnung grösser, Laibung füllt)
...
function openingRevealStrips(a, arr) {   // Laibung: 1,5 cm Rahmen sichtbar → Schalung 1,5 cm (Schraffur) → Rest Dämmung bis Rahmen; innen Putz/Brett
  ...
  const drawReveal = (layers, sideOut, sgn, slopeCm, bv, edge, grow) => {   // EINE Laibungsseite: dünne Laibung am Rahmen; bv = „Rahmen sichtbar"; grow = Öffnung vergrössern (Wand zurück, Laibung füllt)
    if (!layers || !layers.length) return;
    if (a.noSillReveal && sgn < 0) return;   // Schwelle bei Fensterbank überspringen (nur im Schnitt-sa relevant)
    const lapPt = Math.max(cmToPts(0.3), Math.min(hw * 0.92, (a.frameW || cmToPts(10)) - cmToPts(bv != null ? bv : boardVisCm))), growPt = cmToPts(grow || 0);   // seitliche Lappung + Öffnungs-Vergrösserung (füllt bis zur zurückversetzten Wandkante = sIn −growPt)
    ...
    ord.forEach((L, idx) => {
      ...
      if (idx === ord.length - 1 && growPt > 0) sOut = Math.min(sOut, -growPt);   // letzte (wandseitige) Schicht füllt die vergrösserte Öffnung bis zur zurückgeschnittenen Wandkante
  ...
  for (const sgn of [-1, 1]) {   // links (sgn -1, Kante L) und rechts (sgn +1, Kante R) je eigene Laibung; Fallback = globale Liste
    const edge = sgn < 0 ? 'L' : 'R', er = a.reveals && a.reveals[edge], slope = (er && er.slope) || 0, bv = er && er.boardVis != null ? er.boardVis : boardVisCm, grow = er && er.grow ? er.grow : 0;
    if (anType !== 'innen') drawReveal(openingEdgeLayers(a, edge, 'i') || innerLayers, false, sgn, slope, bv, edge, grow);
    if (anType !== 'aussen') drawReveal(openingEdgeLayers(a, edge, 'o') || outerLayers, true, sgn, slope, bv, edge, grow);
  }
  if (anType !== 'none') {
...
function openRevealLayerPop(pv, a, revAttr, cx, cy) {   // Inline-Editor für EINE Laibungsschicht (im Grundriss angeklickt): Material/Dicke + „Rahmen sichtbar" dieser Kante
  ...
  const bvv = document.createElement('input'); bvv.type = 'number'; bvv.min = '-20'; bvv.max = '30'; bvv.step = '0.1'; bvv.value = (a.reveals[edge].boardVis != null ? a.reveals[edge].boardVis : (a.boardVis != null ? a.boardVis : 1)); bvv.style.width = '62px'; bvv.title = 'cm Rahmen sichtbar (Standard 1). Negativ = Laibung deckt mehr Rahmen'; bvv.onchange = () => { const v = parseFloat((bvv.value || '').replace(',', '.')), nv = isNaN(v) ? 1 : v; eachEdge(er => { er.boardVis = nv; }); upd(); }; mk('Rahmen sichtbar (cm)', bvv, () => { eachEdge(er => { er.boardVis = 1; }); bvv.value = 1; upd(); });
  const gr = document.createElement('input'); gr.type = 'number'; gr.min = '0'; gr.max = '60'; gr.step = '0.5'; gr.value = (a.reveals[edge].grow || 0); gr.style.width = '62px'; gr.title = 'Laibung über den Rahmen hinaus verlängern: die Wand wird zurückgeschnitten (Öffnung grösser), die Laibung füllt den Bereich (z. B. 20 cm)'; gr.onchange = () => { const v = parseFloat((gr.value || '').replace(',', '.')), nv = isNaN(v) ? 0 : Math.max(0, v); eachEdge(er => { er.grow = nv; }); upd(); }; mk('Laibung verlängern (cm)', gr, () => { eachEdge(er => { er.grow = 0; }); gr.value = 0; upd(); });
...
function sectionPrimitives(a, arr) {
  ...
  if (!a.noDims) {   // Höhen-Masketten wie im Grundriss: zwei Ketten (links innen=Fertig/Licht „F", rechts aussen=Rohbau „R"), wandabhängig, gleicher Abstand, geschnittene Fenster/Türen mitgenommen
    ...
    const dimChain = (xLine, side, fertig, tag) => {   // side<0 links (Text links), side>0 rechts; fertig = Licht (Laibung abgezogen)
      ...
      for (let i = 0; i < hs.length - 1; i++) { const seg = hs[i + 1] - hs[i]; if (seg < 0.02) continue; out.push({ t: 'text', x: xLine + side * 11, y: (Yh(hs[i]) + Yh(hs[i + 1])) / 2, text: fmtLen(seg / perPt), col, size: 11, ang: -90, mid: true }); }   // Segment-Mass parallel zur Masslinie (vertikal, von rechts lesbar), Schriftgrösse 11 wie im Grundriss
    };
...
function openingResolve(a, pv) {   // Position/Winkel/Dicke aus der zugehörigen Wand ableiten (Öffnung läuft mit)
  ...
  if ((a.kind === 'window' || a.kind === 'door') && w.layers && w.layers.length) {   // STANDARD-Laibung: übernimmt die Deckschicht (Material + Stärke) – innen innerste, aussen äusserste Wandschicht; deckt bis 1cm vom Rahmen
    ...
    if (!a.reveals) { const cp = arr2 => (Array.isArray(arr2) ? arr2 : []).map(L => ({ mat: L.mat, t: L.t, gap: L.gap, prio: L.prio })); a.reveals = {}; for (const e of ['L', 'R', 'T', 'B']) a.reveals[e] = { in: cp(a.revealLining), out: cp(a.revealLiningOut), boardVis: (a.boardVis != null ? a.boardVis : 1) }; }   // jede Kante eigenständig + eigenes „Rahmen sichtbar"
  }
}
