...
function drawOpening(svg, a, arr) {
  ...
  if (seld) for (const st of revHits) { const gc = revealGroupColor(a, st.edge, st.side === 'i' ? 'in' : 'out'); svg.appendChild(addHoverHL(svgEl('polygon', { class: 'rev-hit', points: st.poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: gc, 'fill-opacity': 0.08, stroke: gc, 'stroke-width': 1, 'stroke-opacity': 0.75, 'stroke-dasharray': '4 3', 'vector-effect': 'non-scaling-stroke', 'data-id': a.id, 'data-rev': st.edge + ':' + st.side + ':' + st.li }))); }   // Laibungs-Schichten: gestrichelte Gruppenfarben-Kontur + ganz leichte Füllung (Mittelweg)
...
function sectionCutOpening(out, X, Yh, distPt, appW, o, H, perPt, wall, flip, noDims, mullion, revealOnly) {   // revealOnly = nur Laibung/Schichteinzug zeichnen (Rahmen/Glas/Bank kommen aus Solids)
  ...
  if (layered) {   // Laibung = voller Wandaufbau (wie im Grundriss): Mauerwerk+Dämmung+Putz laufen als Sturz-Untersicht / Brüstungs-Oberseite (schraffiert) bis zum Rahmen; Lining-Lappung + Rahmen/Glas zeichnen darüber
    ...
    for (const [yA, yB] of bands) { const yT = Math.min(yA, yB), hh = Math.abs(yB - yA); if (hh < 1) continue; let acc = 0; for (const L of wls) { const m0 = -1 + 2 * acc / tot, m1 = -1 + 2 * (acc + L.t) / tot; acc += L.t; if (L.mat === 'luft') continue; const xa = cx + ht2 * m0, xb = cx + ht2 * m1, x = Math.min(xa, xb), wd = Math.abs(xb - xa), mt = WALL_MATS[L.mat] || {}; if (wd < 0.3) continue; out.push({ t: 'rect', x, y: yT, w: wd, h: hh, fill: mt.fill || '#fff', stroke: mt.color || '#1c242c', sw: 0.4 }); if (!simpleMode && mt.hatch) sectionBandHatch(out, x, yT, wd, hh, L.mat, null); } }   // voller Wandaufbau als Sturz-/Brüstungs-Untersicht (Basis); die Lining-Lappung (openingRevealStrips) verschneidet 1:1 wie im Grundriss darüber
  }
...
function drawSection(svg, a, arr) {
  const revLayer = () => { const rh = _secCache[a.id] && _secCache[a.id]._revHits; if (rh) for (const h of rh) { const at = { class: h.frame ? 'frame-hit' : 'rev-hit', points: h.pts.map(q => q[0].toFixed(2) + ',' + q[1].toFixed(2)).join(' '), fill: h.gcol || 'transparent', 'data-id': h.oid }; if (h.gcol) { at.fill = h.gcol; at['fill-opacity'] = 0.08; at.stroke = h.gcol; at['stroke-width'] = 1; at['stroke-opacity'] = 0.75; at['stroke-dasharray'] = '4 3'; at['vector-effect'] = 'non-scaling-stroke'; } if (h.frame) at['data-frame'] = '1'; else at['data-rev'] = h.rev; svg.appendChild(addHoverHL(svgEl('polygon', at))); } };   // klickbare Laibungsschichten (Sturz/Schwelle) + Rahmen, farbcodiert nach Zugehörigkeit (konsistent zum Grundriss)
