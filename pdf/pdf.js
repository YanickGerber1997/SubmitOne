...
// Inspector-Baustein „Position/Notiz": NPK-Positionsnummer + freie Bemerkung (fliessen in Liste + Ausschreibung)
function belagNoteHtml(a) {
  return '<div class="insp-row"><span class="insp-lbl">NPK-Pos.</span><input class="insp-num" style="width:134px" id="bNpk" value="' + ((a.npk || '').replace(/"/g, '&quot;')) + '" placeholder="z. B. 663.211"></div>'
    + '<div class="insp-row"><span class="insp-lbl">Bemerkung</span><input class="insp-num" style="width:134px" id="bBem" value="' + ((a.bemerkung || '').replace(/"/g, '&quot;')) + '" placeholder="frei – erscheint im Auszug"></div>';
}
function bindBelagNote(body, a) {
  const t = (id, key) => { const el = body.querySelector(id); if (el) el.onchange = () => { const v = (el.value || '').trim(); if (v) a[key] = v; else delete a[key]; markDirty(); renderList(); }; };
  t('#bNpk', 'npk'); t('#bBem', 'bemerkung');
}
...
function drawWallFaceElevation(svg, a, pv) {
  ...
  g.appendChild(svgEl('polygon', { points: quad, fill: '#ffffff', 'fill-opacity': 0.88, stroke: col, 'stroke-width': 1.2, 'vector-effect': 'non-scaling-stroke', 'data-id': a.id, 'data-wfgrid': 1, style: 'cursor:grab', title: 'Wand-Ansicht ' + (Math.round(len * perPt * 100) / 100).toFixed(2).replace('.', ',') + '×' + (Math.round(h * 100) / 100).toFixed(2).replace('.', ',') + ' m · Platten ' + b.tileW + '×' + b.tileH + ' cm · ziehen = Einteilung verschieben, Fenster ziehen = verschieben' }));
...
function wfHoverMove(ev) {
  ...
  lab(A[0] + ux * jU + nx * (topPts + 8 / pv.scale), A[1] + uy * jU + ny * (topPts + 8 / pv.scale), (Math.round(jU * perPt * 100) / 100).toFixed(2).replace('.', ',') + ' m');
...
// Wand-Ansicht: Platteneinteilung verschieben – Ziehen ändert den Raster-Versatz (offU entlang Wand, offN quer)
function startGridShift(pv, e, annoId) {
...
function startWinResize(pv, e, annoId, wi) {
  ...
  const move = ev => { moved = true; const q = evtToPage(pv, ev), rx = q.x - a.x1, ry = q.y - a.y1;
    ...
    const sillPts = (f.sill != null ? f.sill : 0.9) / perPt, hpts = Math.max(0.05 / perPt, (rx * nx + ry * ny) - gap - sillPts);
    f.h = Math.max(0.1, Math.round(hpts * perPt * 100) / 100);
    requestDraw(pv);
  };
  const up = () => { document.removeEventListener('pointermove', move); document.removeEventListener('pointerup', up); if (moved) { saveState(); if (typeof renderList === 'function') renderList(); } else if (undoStack.length) { undoStack.pop(); drawAnnos(pv); } };
  document.addEventListener('pointermove', move); document.addEventListener('pointerup', up);
...
function fillSelectionInspector(body) {   // „Auswahl"-Tab: Einstellungen des gewählten Bauteils (wie ein Inspector)
  ...
  if (a.type === 'measure' && a.wallface) {
    ...
    html += belagMaterialHtml(b) + leistHtml(a) + belagNoteHtml(a);
    ...
    body.innerHTML = html;
    bindBelagMaterial(body, a, b); bindLeist(body, a); bindBelagNote(body, a);
...
function fillBelagList(bodyEl) {   // Boden-/Wandbeläge mit Flächen, Platten & Summen
  ...
  if (walls.length) html += '<h4 style="margin-top:14px">Wandflächen (' + walls.length + ')</h4><table class="qty-tab"><thead><tr><th>Wand</th><th style="text-align:right">L×H</th><th style="text-align:right">Fläche</th><th style="text-align:right">Platten</th></tr></thead><tbody>'
    + walls.map(r => '<tr><td>' + (_htmlEsc(r.name) || '–') + (r.aufbau ? '<div style="font-size:10px;color:var(--ink-soft)">' + _htmlEsc(r.aufbau) + '</div>' : '') + (r.verputzM2 ? '<div style="font-size:10px;color:var(--ink-soft)">+ ' + _htmlEsc(r.oben || 'Verputz') + ' oben ' + fmt(r.verputzM2) + ' m²</div>' : '') + (r.a && r.a.bemerkung ? '<div style="font-size:10px;color:var(--ink-soft)">' + _htmlEsc(r.a.bemerkung) + '</div>' : '') + '</td><td style="text-align:right;white-space:nowrap">' + fmt(r.lenM) + '×' + (r.topHr != null ? fmt(r.topHl) + '/' + fmt(r.topHr) : fmt(r.topHl != null ? r.topHl : r.h)) + '</td><td style="text-align:right;white-space:nowrap">' + fmt(r.m2) + ' m²</td><td style="text-align:right">' + r.tiles + '</td></tr>').join('')
  ...
  { const vp = walls.filter(r => r.verputzM2 > 0); if (vp.length) html += '<h4 style="margin-top:14px">Verputz / Wandabschluss (' + vp.length + ')</h4><table class="qty-tab"><thead><tr><th>Wand</th><th>Art</th><th style="text-align:right">Fläche</th></tr></thead><tbody>'
    ...
    + '</tbody><tfoot><tr><th style="text-align:right" colspan="2">Summe</th><th style="text-align:right;white-space:nowrap">' + fmt(vp.reduce((s, r) => s + r.verputzM2, 0)) + ' m²</th></tr></tfoot></table>'; }
...
function buildBelagTableHtml(floors, walls, price, anschluesse, extras, nischen) {
  ...
  const npk = r => (r.a && r.a.npk) ? 'NPK ' + r.a.npk + ' · ' : '';   // NPK-Positionsnummer vor den Beschrieb
  const bem = r => (r.a && r.a.bemerkung) ? ' · ' + r.a.bemerkung : '';   // freie Bemerkung angehängt
  if (floors.length) { sec++; rows += '<tr><td colspan="' + cols + '"><strong>' + sec + '  Bodenbeläge</strong></td></tr>'; floors.forEach((r, i) => rows += posRow(sec + '.' + (i + 1), npk(r) + (r.name || 'Bodenbelag') + ' · Platten ' + r.b.tileW + '×' + r.b.tileH + mat(r) + ug(r) + (r.aufbau ? ' · ' + r.aufbau : '') + (r.cutM2 ? ' · netto (abzügl. Aussparungen ' + fmt(r.cutM2) + ' m²)' : '') + bem(r), r.m2, 'm²')); rows += zt('Bodenbeläge', floors.reduce((s, r) => s + r.m2, 0), 'm²'); }
  ...
  if (walls.length) { sec++; rows += '<tr><td colspan="' + cols + '"><strong>' + sec + '  Wandflächen</strong></td></tr>'; walls.forEach((r, i) => rows += posRow(sec + '.' + (i + 1), npk(r) + (r.name || 'Wandbelag') + ' · ' + wfH(r) + ' · Platten ' + r.b.tileW + '×' + r.b.tileH + mat(r) + ug(r) + (r.aufbau ? ' · ' + r.aufbau : '') + (r.fenM2 ? ' · netto (abzügl. Öffnungen ' + fmt(r.fenM2) + ' m²)' : '') + bem(r), r.m2, 'm²')); rows += zt('Wandflächen', walls.reduce((s, r) => s + r.m2, 0), 'm²'); }
...
function selfTest() {   // prüft die Kern-Rechenpfade (kein DOM nötig); fängt Regressionen
  ...
  try {
    ...
    A('Wandbelag Fenster → Netto + Laibung/Bank/Sturz separat', () => { const sa = annos, sd = docScale; try { docScale = { perPt: 0.01, label: 't' }; annos = { 1: [{ type: 'measure', wallface: true, height: 2.5, x1: 0, y1: 0, x2: 400, y2: 0, ans: { boden: false, decke: false, wand: 0 }, fenster: [{ w: 1.2, h: 1.4, sill: 0.9 }] }] }; const d = belagData(); const w = d.walls[0], bk = {}; d.anschluesse.forEach(r => bk[r.kat] = (bk[r.kat] || 0) + r.lenM); return (Math.abs(w.m2 - (10 - 1.68)) < 1e-6 && Math.abs(bk.bank - 1.2) < 1e-6 && Math.abs(bk.sturz - 1.2) < 1e-6 && Math.abs(bk.laibung - 2.8) < 1e-6) ? '' : JSON.stringify({ n: w.m2, bk }); } finally { annos = sa; docScale = sd; } });
