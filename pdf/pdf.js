...
function openingRevealStrips(a, arr) {   // Laibung: 1,5 cm Rahmen sichtbar → Schalung 1,5 cm (Schraffur) → Rest Dämmung bis Rahmen; innen Putz/Brett
  ...
  const drawReveal = (layers, sideOut, sgn, slopeCm, bv, edge) => {   // EINE Laibungsseite: dünne Laibung am Rahmen; bv = „Rahmen sichtbar" dieser Kante; tag jede Schicht (edge/side/li) für Klick im Grundriss
    ...
    ord.forEach(L => {
      ...
      const mB2 = mFace + (-dirF) * (over ? cmToPts(over) / ht : 0), mA2 = (len != null) ? Math.max(-1.15, Math.min(1.15, mFace + dirF * cmToPts(len) / ht)) : mFrame, m0 = Math.min(mA2, mB2), m1 = Math.max(mA2, mB2);   // Länge (len) darf über den Rahmen hinaus (z. B. Mauerwerk tiefer); Überstand/Rücksprung (over)
...
function openRevealLayerPop(pv, a, revAttr, cx, cy) {   // Inline-Editor für EINE Laibungsschicht (im Grundriss angeklickt): Material/Dicke + „Rahmen sichtbar" dieser Kante
  ...
  const bvv = document.createElement('input'); bvv.type = 'number'; bvv.min = '-20'; bvv.max = '30'; bvv.step = '0.1'; bvv.value = (a.reveals[edge].boardVis != null ? a.reveals[edge].boardVis : (a.boardVis != null ? a.boardVis : 1)); bvv.style.width = '62px'; bvv.title = 'cm Rahmen sichtbar. Negativ = Laibung ragt weiter über den Rahmen hinaus'; bvv.onchange = () => { const v = parseFloat((bvv.value || '').replace(',', '.')), nv = isNaN(v) ? 1 : v; eachEdge(er => { er.boardVis = nv; }); upd(); }; mk('Rahmen sichtbar (cm)', bvv, () => { eachEdge(er => { er.boardVis = 1; }); bvv.value = 1; upd(); });
