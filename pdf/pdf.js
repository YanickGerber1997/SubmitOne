...
function drawSection(svg, a, arr) {
  const revLayer = () => { const rh = _secCache[a.id] && _secCache[a.id]._revHits; if (rh) for (const h of rh) { const at = { class: h.frame ? 'frame-hit' : 'rev-hit', points: h.pts.map(q => q[0].toFixed(2) + ',' + q[1].toFixed(2)).join(' '), fill: h.gcol || 'transparent', 'data-id': h.oid }; if (h.gcol) { at['fill-opacity'] = 0.22; at.stroke = h.gcol; at['stroke-width'] = 1.2; } if (h.frame) at['data-frame'] = '1'; else at['data-rev'] = h.rev; svg.appendChild(addHoverHL(svgEl('polygon', at))); } };   // klickbare Laibungsschichten (Sturz/Schwelle) + Rahmen, farbcodiert nach Zugehörigkeit (konsistent zum Grundriss)
