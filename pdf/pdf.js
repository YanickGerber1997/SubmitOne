...
function build3DScene(host, walls, arr, opts) {
  ...
  if (opts.onEdit) {   // Griffe: Wand-Endpunkte (Kugel, Snapping) + Wand-Mittelpunkt (Quader) + Möbel/Stützen-Mittelpunkt → ändern DAS Bauteil-Objekt
    const hR = Math.max(0.07, span * 0.013), hGeo = new THREE.SphereGeometry(hR, 14, 14), midGeo = new THREE.BoxGeometry(hR * 1.7, hR * 0.5, hR * 1.7), blkGeo = new THREE.BoxGeometry(hR * 2.2, hR * 0.5, hR * 2.2);
    const cEnd = 0x2f7be4, cMid = 0x6b7280, cBlk = 0x8a5cc4, cHot = 0xf08a24, cSnap = 0x2fae4e;
    ...
    const onMove = ev => {
      ...
      if (end === 'mid' || end === 'block') { const dx = hitP.x - startW.x, dz = hitP.z - startW.z; drag.position.x = startW.x + dx; drag.position.z = startW.z + dz; drag.material.color.setHex(cHot); if (end === 'mid') setPrev(M(orig.x1 - cx) + dx, M(orig.y1 - cy) + dz, M(orig.x2 - cx) + dx, M(orig.y2 - cy) + dz, y); else prevLine.visible = false; }
