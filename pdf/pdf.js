...
function openingRevealStrips(a, arr) {   // Laibung: 1,5 cm Rahmen sichtbar → Schalung 1,5 cm (Schraffur) → Rest Dämmung bis Rahmen; innen Putz/Brett
  ...
  const userLin = Array.isArray(a.revealLining) && a.revealLining.length ? a.revealLining.map(L => [L.mat, L.t, (L.depth == null ? 1 : L.depth)]) : null;   // eigene Laibungsschichten (Material/Dicke/Tiefe), sonst Preset
  const innerLining = userLin || (rt0 === 'aussen' ? [[lN.mat, Math.min(3, ptsToCm(lN.t)), 1]] : (REVEAL_LINING[rt0] || [[l0.mat, ptsToCm(l0.t)]]).map(e => [e[0], e[1], 1]));   // putz = innerste Wandschicht reingezogen; aussen = äusserste; sonst feste Konstruktion
  if (anType !== 'innen' && Math.abs((fmA - oneCm) + 1) > 0.02) { let sAcc = 0; const mEndFull = fmA - oneCm; for (const [mat, tcm, dep] of innerLining) { const mt = LINING_MAT[mat] || WALL_MATS[mat] || {}, tw = Math.min(0.42, cmToPts(tcm) / hw), sk = mt.stroke || mt.color || '#1c242c', mEnd = -1 + Math.max(0.02, Math.min(1, dep)) * (mEndFull + 1); for (const sgn of [-1, 1]) { if (a.noSillReveal && sgn === -1) continue; const s0 = sgn * (1 - sAcc), s1 = sgn * (1 - sAcc - tw); strips.push({ poly: [corner(s0, -1), corner(s1, -1), corner(s1, mEnd), corner(s0, mEnd)], fill: mt.fill || '#fff', stroke: sk, hatch: mt.hatch ? bandHatch(Math.min(s0, s1), Math.max(s0, s1), -1, mEnd, corner, hw, ht, stepS) : null }); } sAcc += tw; } }   // Innen-Laibung: je Schicht eigene Tiefe (dep) in die Laibung; ohne Innenanschlag
    ...
    { // --- Eigene Laibungsschichten (frei: 1/2/3… · Material/Dicke/Tiefe je Schicht) ---
      ...
      if (!Array.isArray(RL) || !RL.length) {
        ...
        RL.forEach((L, i) => {
          ...
          l2.appendChild(rg); l2.appendChild(pl); row.appendChild(l2); side.appendChild(row);
        });
