...
function openingRevealStrips(a, arr) {   // Laibung: 1,5 cm Rahmen sichtbar → Schalung 1,5 cm (Schraffur) → Rest Dämmung bis Rahmen; innen Putz/Brett
  ...
  const userLin = Array.isArray(a.revealLining) && a.revealLining.length ? a.revealLining.map(L => [L.mat, L.t, (L.depth == null ? 1 : L.depth), (L.protrude || 0)]) : null;   // eigene Laibungsschichten (Material/Dicke/Tiefe/Überstand), sonst Preset
  ...
  if (anType !== 'innen' && Math.abs((fmA - oneCm) + 1) > 0.02) { let sAcc = 0; const mEndFull = fmA - oneCm; for (const [mat, tcm, dep, prot] of innerLining) { const mt = LINING_MAT[mat] || WALL_MATS[mat] || {}, tw = Math.min(0.42, cmToPts(tcm) / hw), sk = mt.stroke || mt.color || '#1c242c', mEnd = -1 + Math.max(0.02, Math.min(1, dep)) * (mEndFull + 1), mStart = -1 - (prot ? cmToPts(prot) / ht : 0); for (const sgn of [-1, 1]) { if (a.noSillReveal && sgn === -1) continue; const s0 = sgn * (1 - sAcc), s1 = sgn * (1 - sAcc - tw); strips.push({ poly: [corner(s0, mStart), corner(s1, mStart), corner(s1, mEnd), corner(s0, mEnd)], fill: mt.fill || '#fff', stroke: sk, hatch: mt.hatch ? bandHatch(Math.min(s0, s1), Math.max(s0, s1), mStart, mEnd, corner, hw, ht, stepS) : null }); } sAcc += tw; } }   // Innen-Laibung: je Schicht Tiefe (dep) + Überstand (prot, ragt über die Wandfläche)
  const rtOut = a.revealOuter || '';   // Aussen-Laibung (Putz/Gips/Holz/Stahl/Alu) – spiegelbildlich zur Innen-Laibung
  if (rtOut && anType !== 'aussen' && Math.abs((fmB + oneCm) - 1) > 0.02) { const outerLining = rtOut === 'putz' ? [[lN.mat, Math.min(3, ptsToCm(lN.t))]] : (REVEAL_LINING[rtOut] || [[lN.mat, Math.min(3, ptsToCm(lN.t))]]); let sAcc = 0; for (const [mat, tcm] of outerLining) { const mt = LINING_MAT[mat] || WALL_MATS[mat] || {}, tw = Math.min(0.42, cmToPts(tcm) / hw), sk = mt.stroke || mt.color || '#1c242c'; for (const sgn of [-1, 1]) { if (a.noSillReveal && sgn === -1) continue; const s0 = sgn * (1 - sAcc), s1 = sgn * (1 - sAcc - tw); strips.push({ poly: [corner(s0, 1), corner(s1, 1), corner(s1, fmB + oneCm), corner(s0, fmB + oneCm)], fill: mt.fill || '#fff', stroke: sk, hatch: mt.hatch ? bandHatch(Math.min(s0, s1), Math.max(s0, s1), fmB + oneCm, 1, corner, hw, ht, stepS) : null }); } sAcc += tw; } }
  ...
  if (outerPos.length) {   // jede Aussenschicht läuft ununterbrochen aus der Wand (an ihrer m-Position) bis ans Laibungsbrett; Hinterlüftung bleibt offener Spalt
    ...
    for (const sgn of [-1, 1]) {
      if (a.noSillReveal && sgn === -1) continue;   // an der Schwelle (Fensterbank) keine umlaufende Laibung/Brett
...
function openLaibungEditor(a, pv) {   // interaktives Laibungs-Detail: reinzoomen, Ziehgriffe + Regler, live in den Plan
  ...
  function renderElev(el, side, getVb, setVb) {   // Ansicht innen/aussen – die ganze WAND mit allen Fenstern/Türen (Kontext), aktuelles markiert
    ...
    for (const o2 of ops) {
      ...
      else { const ro = o2.revealOuter || ''; if (ro && REVEAL_LINING[ro]) { for (const [mat, tcm] of REVEAL_LINING[ro]) rings.push({ mat, w: cmToPts(tcm) }); } else if (ro === 'putz') rings.push({ mat: lyrN ? lyrN.mat : 'putz', w: lapClad }); else { if (facLy) rings.push({ mat: facLy.mat, w: lapClad }); const wood = facLy && ['holz', 'schalung', 'konter'].includes(facLy.mat); if (wood && o2.kind === 'window' && (o2.boardW == null || o2.boardW > 0)) rings.push({ mat: o2.boardMat || 'holz', w: cmToPts(o2.boardW != null ? o2.boardW : 2) }); } }   // Aussen-Laibung nach revealOuter (Standard = Verkleidung + Brett bei Holzfassade)
      let cum = 0;
  ...
  function buildCtrls() {
    ...
    { // --- Eigene Laibungsschichten (frei: 1/2/3… · Material/Dicke/Tiefe je Schicht) ---
      ...
      if (!Array.isArray(RL) || !RL.length) {
        const b = document.createElement('button'); b.className = 'btn'; b.textContent = '⊞ Eigene Laibungsschichten'; b.style.cssText = 'width:100%;margin:5px 0 2px'; b.title = 'Laibung aus frei definierbaren Schichten (Material/Dicke/Tiefe) – Start = Wandschichten, danach frei änder-/lösch-/ergänzbar';
        b.onclick = () => { a.revealLining = prefill(); render(); buildCtrls(); drawAnnos(pv); saveState(); }; side.appendChild(b);
      } else {
        const hint = document.createElement('div'); hint.className = 'lab-row'; hint.innerHTML = '<span style="color:#46503f">innen→aussen · Tiefe = wie weit in die Laibung</span>'; side.appendChild(hint);
        RL.forEach((L, i) => {
          ...
          const del = document.createElement('button'); del.className = 'btn'; del.textContent = '✕'; del.style.cssText = 'padding:0 8px'; del.title = 'Schicht löschen'; del.onclick = () => { RL.splice(i, 1); if (!RL.length) a.revealLining = null; render(); buildCtrls(); drawAnnos(pv); saveState(); };
          l1.appendChild(ms); l1.appendChild(tn); l1.appendChild(del); row.appendChild(l1);
          const l2 = document.createElement('div'); l2.className = 'lab-line';
          const rg = document.createElement('input'); rg.type = 'range'; rg.min = '5'; rg.max = '100'; rg.step = '5'; rg.value = Math.round((L.depth == null ? 1 : L.depth) * 100); rg.style.flex = '1'; rg.title = 'Tiefe in die Laibung (%)';
          const pl = document.createElement('b'); pl.textContent = rg.value + '%'; pl.style.minWidth = '36px';
          rg.oninput = () => { L.depth = parseInt(rg.value) / 100; pl.textContent = rg.value + '%'; render(); drawAnnos(pv); }; rg.onchange = () => saveState();
          const pr = document.createElement('input'); pr.type = 'number'; pr.min = '0'; pr.max = '10'; pr.step = '0.5'; pr.value = (L.protrude || 0); pr.style.width = '46px'; pr.title = 'Überstand über die Wandfläche (cm) – z. B. Brett ragt über den Putz'; pr.onchange = () => { const v = parseFloat((pr.value || '').replace(',', '.')); L.protrude = isNaN(v) ? 0 : v; render(); drawAnnos(pv); saveState(); };
          const prl = document.createElement('span'); prl.textContent = '↥cm'; prl.style.cssText = 'font-size:10px;color:#7a8366';
          l2.appendChild(rg); l2.appendChild(pl); l2.appendChild(pr); l2.appendChild(prl); row.appendChild(l2); side.appendChild(row);
        });
        ...
        const wb = document.createElement('button'); wb.className = 'btn'; wb.textContent = '⟳ Wand'; wb.style.flex = '1'; wb.title = 'Aus den Wandschichten neu übernehmen'; wb.onclick = () => { a.revealLining = prefill(); render(); buildCtrls(); drawAnnos(pv); saveState(); };
        bar.appendChild(add); bar.appendChild(wb); side.appendChild(bar);
        const rs = document.createElement('button'); rs.className = 'btn'; rs.textContent = 'Standard (zurücksetzen)'; rs.style.cssText = 'width:100%;margin-top:3px'; rs.onclick = () => { a.revealLining = null; render(); buildCtrls(); drawAnnos(pv); saveState(); }; side.appendChild(rs);
      }
    }
    if (win) {
...
function build3DScene(host, walls, arr, opts) {
  ...
  return { dispose: () => { alive = false; cancelAnimationFrame(raf); cleanups.forEach(fn => { try { fn(); } catch (_) { } }); window.removeEventListener('resize', onResize); controls.dispose(); renderer.dispose(); host.innerHTML = ''; }, setView, snapshot, exportObj: () => exportSceneObj(scene), setRotate: on => { controls.autoRotate = !!on; }, getRotate: () => controls.autoRotate, setSun: setSun3D, setSunDir, camState: () => ({ p: camera.position.toArray(), t: controls.target.toArray() }), setAddMode: v => editSetAddMode(v) };
}
