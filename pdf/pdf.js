...
function drawOpening(svg, a, arr) {
  ...
  if (seld) for (const f of (P.fills || [])) { if (f.role !== 'frame') continue; svg.appendChild(addHoverHL(svgEl('polygon', { class: 'frame-hit', points: f.poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: 'transparent', 'data-id': a.id, 'data-frame': '1' }))); }   // Rahmen anklickbar (nur bei gewähltem Fenster) – keine Füllung
  if (seld) for (const st of revHits) { const gc = revealGroupColor(a, st.edge, st.side === 'i' ? 'in' : 'out'); svg.appendChild(addHoverHL(svgEl('polygon', { class: 'rev-hit', points: st.poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: 'transparent', stroke: gc, 'stroke-width': 0.6, 'stroke-opacity': 0.5, 'stroke-dasharray': '3 2', 'vector-effect': 'non-scaling-stroke', 'data-id': a.id, 'data-rev': st.edge + ':' + st.side + ':' + st.li }))); }   // Laibungs-Schichten: nur ganz dünne, gestrichelte Gruppenfarben-Kontur
  ...
  if (seld) { const seen = {}; const edgeStrip = (A, B, color, attr) => { const dx = B[0] - A[0], dy = B[1] - A[1], L = Math.hypot(dx, dy) || 1, ox = -dy / L * 4, oy = dx / L * 4, poly = [[A[0] + ox, A[1] + oy], [B[0] + ox, B[1] + oy], [B[0] - ox, B[1] - oy], [A[0] - ox, A[1] - oy]]; const z = svgEl('polygon', Object.assign({ points: poly.map(p => p[0].toFixed(2) + ',' + p[1].toFixed(2)).join(' '), fill: 'transparent', stroke: 'none', 'data-id': a.id }, attr)); z.style.cursor = 'grab'; svg.appendChild(z); };   // unsichtbare Ziehkante (nur Hit-Zone, etwas breiter zum Greifen)
    for (const st of revHits) { const k2 = st.edge + ':' + st.side; if (seen[k2]) continue; seen[k2] = 1; const q = st.poly; if (q.length < 4) continue; const sgn = st.edge === 'R' ? 1 : -1, ca = Math.cos(a.ang || 0), sa = Math.sin(a.ang || 0), key = st.edge + ':' + st.side + ':' + st.li;
      _revtHandles[a.id + '|' + key] = { dirx: -sgn * ca, diry: -sgn * sa }; edgeStrip(q[0], q[3], '#1a9a4e', { 'data-revt': key });   // grüne Kante = Lappung (Rahmen sichtbar) an der Rahmenkante ziehen
      const fmx = (q[0][0] + q[3][0]) / 2, fmy = (q[0][1] + q[3][1]) / 2, flx = (q[2][0] + q[3][0]) / 2, fly = (q[2][1] + q[3][1]) / 2, odx = flx - fmx, ody = fly - fmy, odl = Math.hypot(odx, ody) || 1; _revoHandles[a.id + '|' + key] = { dirx: odx / odl, diry: ody / odl, li: st.li }; edgeStrip(q[2], q[3], '#d4881f', { 'data-revo': key });   // orange Kante = Überstand an der Wandflächenkante ziehen
    } }
...
function sectionCutOpening(out, X, Yh, distPt, appW, o, H, perPt, wall, flip, noDims, mullion, revealOnly) {   // revealOnly = nur Laibung/Schichteinzug zeichnen (Rahmen/Glas/Bank kommen aus Solids)
  ...
  if (layered) {   // Laibung = voller Wandaufbau (wie im Grundriss): Mauerwerk+Dämmung+Putz laufen als Sturz-Untersicht / Brüstungs-Oberseite (schraffiert) bis zum Rahmen; Lining-Lappung + Rahmen/Glas zeichnen darüber
    const wls = flip ? wall.layers.slice().reverse() : wall.layers, tot = (wls || []).reduce((s, l) => s + l.t, 0) || appW, revM = ptsToCm(o.frameW || cmToPts(10)) / 100;
    const bands = [[Yh(head), Yh(Math.max(sill, head - revM))]];   // Sturz (Untersicht)
    if (!(o.kind === 'window' && o.bank !== false)) bands.push([Yh(Math.min(head, sill + revM)), Yh(sill)]);   // Schwelle/Brüstung (entfällt bei Fensterbank)
    const CORE = ['mauerwerk', 'beton', 'kalksand', 'klinker', 'staender', 'stahl'];   // Tragschicht stoppt an der Laibungssicht (zeigt nur die Sturz-Untersicht); Dämmung/Putz laufen bis zum Rahmen
    const erT = o.reveals && o.reveals.T, bvIn = erT && erT.boardVisIn != null ? erT.boardVisIn : (o.boardVis != null ? o.boardVis : 1), lapInner = Math.max(cmToPts(0.5), (o.frameW || cmToPts(10)) - cmToPts(bvIn));   // Lining-Lappung innen (= Stopp-Tiefe der Tragschicht)
    for (const [yA, yB] of bands) { const yT = Math.min(yA, yB), hh = Math.abs(yB - yA); if (hh < 1) continue; const lapPx = Math.min(hh, lapInner / perPt); let acc = 0; for (const L of wls) { const m0 = -1 + 2 * acc / tot, m1 = -1 + 2 * (acc + L.t) / tot; acc += L.t; if (L.mat === 'luft') continue; const lh = CORE.includes(L.mat) ? lapPx : hh; if (lh < 0.5) continue; const xa = cx + ht2 * m0, xb = cx + ht2 * m1, x = Math.min(xa, xb), wd = Math.abs(xb - xa), mt = WALL_MATS[L.mat] || {}; if (wd < 0.3) continue; out.push({ t: 'rect', x, y: yT, w: wd, h: lh, fill: mt.fill || '#fff', stroke: mt.color || '#1c242c', sw: 0.4 }); if (!simpleMode && mt.hatch) sectionBandHatch(out, x, yT, wd, lh, L.mat, null); } }
  }
...
function drawSection(svg, a, arr) {
  const revLayer = () => { const rh = _secCache[a.id] && _secCache[a.id]._revHits; if (rh) for (const h of rh) { const at = { class: h.frame ? 'frame-hit' : 'rev-hit', points: h.pts.map(q => q[0].toFixed(2) + ',' + q[1].toFixed(2)).join(' '), fill: h.gcol || 'transparent', 'data-id': h.oid }; if (h.gcol) { at.fill = 'transparent'; at.stroke = h.gcol; at['stroke-width'] = 0.6; at['stroke-opacity'] = 0.5; at['stroke-dasharray'] = '3 2'; at['vector-effect'] = 'non-scaling-stroke'; } if (h.frame) at['data-frame'] = '1'; else at['data-rev'] = h.rev; svg.appendChild(addHoverHL(svgEl('polygon', at))); } };   // klickbare Laibungsschichten (Sturz/Schwelle) + Rahmen, farbcodiert nach Zugehörigkeit (konsistent zum Grundriss)
  ...
  const g = svgEl('g', { 'data-id': a.id }), hdl = [], revHits = [], wallHB = {}, owHits = [];
